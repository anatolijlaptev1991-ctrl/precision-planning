# История изменений

## 1.1.2 — 2026-07-19

- Убраны явные разделения «для человека» и «для агента».
- В начало README и SKILL.md добавлено единое краткое описание функционала и преимуществ перед стандартными Plan mode.
- Добавлено сравнение с подходами Claude Code, Codex, OpenCode и Antigravity со ссылками на их официальные описания.
- Техническая спецификация, CLI и reference tree синхронизированы с `laptev-plan` v5.1.2.
- Подготовлен GitHub Release `v1.1.2` и npm-пакет `1.1.2`.

## 1.1.1 — 2026-07-19

- Описание скилла переведено в двухслойный формат: сначала простое объяснение для человека без IT-фона, затем точный контракт для автономного агента.
- Добавлены понятный словарь терминов и правила чтения нормативного текста.
- Обновлены frontmatter, README, CLI-версия и npm-описание: `laptev-plan` v5.1.1, пакет v1.1.1.
- Функциональная модель планирования не изменена.

## 1.1.0 — 2026-07-19

- Скилл синхронизирован с локальным Hermes Agent: `laptev-plan` v5.1.0.
- Добавлено полное дерево reference-файлов: схемы артефактов, контракт атомарного шага, runtime-протокол, executor detection, Rust-пример и upstream-hermetic checklist.
- Обновлено описание npm-пакета и README под фактическую модель `executor_constraints`.
- Убраны устаревшие описания v4.0 и чекпоинтов `AIO`/`VERIFY` как исполняемых единиц.
- CLI теперь копирует всё дерево `skill/`, а не только `SKILL.md`.
- `precision-planning install --home ...` больше не изменяет глобальную Windows-переменную окружения.
- `status` проверяет версию skill и наличие reference tree.

## 1.0.7 — 2026-07-14

- Удалены все упоминания «Precision Planning» и «v6» из CLI, CHANGELOG.
- CLI-сообщения и help теперь используют «laptev-plan».

## 1.0.6 — 2026-07-14

- README полностью переписан под Версия 4.0 (ЛШ, фазы, чекпоинты, risk matrix, evidence, аудит).
- Обновлено описание npm-пакета.

## 1.0.5 — 2026-07-14

- CLI: поддержка `--version` наравне с `version`.

## 1.0.4 — 2026-07-14

- Скилл заменён на Версия 4.0 — универсальная спецификация с формальными
  контрактами, моделью рисков, бюджетом проверок, машинно-читаемыми
  логами и управляемой деградацией.
- Файловая структура плана `Plan_<name>/` с YAML/NDJSON артефактами.
- Логические шаги (ЛШ), фазы, задачи, чекпоинты, evidence-модель.
- Многомерная модель рисков с risk_tags и матрицей действий.
- Профиль среды исполнения с command policy, path policy, network policy.
- Аудит с адаптивным количеством раундов и классификацией изъянов.
- Регенерация плана при пропуске задач с лимитом и блокировкой.

## 1.0.3 — 2026-07-14

- Каноническое имя скилла изменено на `laptev-plan`, как у `laptev-plan-impl`.
- Скилл теперь устанавливается в общий каталог Hermes `skills/software-development/laptev-plan/`.
- Команда `/laptev-plan` теперь создаётся самим skill scanner во всех сессиях.
- `/laptev_plan` и `/precision-planning` сохранены как совместимые aliases.
- Установщик мигрирует старый рекурсивный alias `/laptev-plan`.

## 1.0.2 — 2026-07-14

- Исправлена доступность команды `/laptev-plan`.
- Установщик теперь регистрирует оба alias: `/laptev-plan` и `/laptev_plan`.
- Сохранено прямое имя скилла `/precision-planning`.
- `status` и `uninstall` теперь проверяют и удаляют оба alias.

## 1.0.0 — 2026-07-14

- Добавлен скилл laptev-plan v4.0.0.
- Добавлен npm-установщик `precision-planning`.
- Добавлена установка скилла в Hermes Agent.
- Добавлена автозагрузка через `HERMES_TUI_SKILLS`.
- Добавлен alias `/laptev_plan`.
- Добавлены команды `install`, `status` и `uninstall`.
- Добавлено сравнение с Claude Code, Google Antigravity, OpenAI Codex, OpenCode и ZCode.
