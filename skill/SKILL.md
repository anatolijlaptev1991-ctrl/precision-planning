...
license: MIT
description: >-
  Используй, когда человек просит составить план, спланировать задачу,
  разбить её на шаги, декомпозировать, создать план внедрения, plan,
  planning, atomic plan или пошаговый план. Для человека этот skill превращает
  обычное описание задачи в понятную карту действий с проверками, рисками и
  откатом. Для автономного агента он создаёт машинно-читаемые YAML/NDJSON-
  контракты: логические шаги, атомарные действия, зависимости, evidence,
  реестр артефактов и readiness gate. Исполнитель описывается только
  измеримыми ограничениями (executor_constraints), а не названием модели.
---

# LAPTEV-PLAN: ПЛАН ЗАДАЧИ, КОТОРЫЙ ПОНЯТЕН ЧЕЛОВЕКУ И ИСПОЛНИМ АГЕНТОМ

## Коротко для человека

Представь карту маршрута. Ты описываешь, **куда нужно прийти**, обычными словами.
Скилл превращает это описание в маршрут: что сделать, в каком порядке, как
проверить результат, что может пойти не так и как вернуться назад.

Скилл не выполняет задачу сам и не принимает решения вместо пользователя.
Он подготавливает безопасный план, который можно передать агенту-исполнителю.

## Коротко для автономного агента

Вход: один или несколько `.md`-файлов с требованиями, процедурой или архитектурой.
Выход: самодостаточная директория `Plan_<plan_name>/` с фазами, задачами,
атомарными шагами, зависимостями, рисками, rollback, evidence, реестром
артефактов, журналами, аудитом и `executor_readiness.yaml`.

Нормативные слова имеют строгий смысл:

- **ОБЯЗАН** — правило нельзя пропустить;
- **ЗАПРЕЩЕНО** — действие недопустимо;
- **МОЖЕТ** — допустимый вариант, а не обязанность;
- поля YAML/NDJSON, закрытые списки и `done_criteria` — машинный контракт;
- обычное объяснение отвечает на вопрос «зачем», контракт отвечает на вопрос
  «что именно проверить».

## Словарь простыми словами

| Термин | Объяснение без IT | Что это значит для агента |
|---|---|---|
| Логический шаг (`LSH`) | Одна мысль из исходного текста | Имеет `source_anchor` и `origin` |
| Атомарный шаг | Одно маленькое действие | Ровно один вызов исполнителя и один `action` |
| Evidence | Чек или квитанция результата | Машинная запись exit code, хешей, проверок и redaction |
| Артефакт | Подписанный результат для следующего участника | Неизменяемая запись в `artifacts_registry.yaml` |
| Risk tag | Ярлык «что может быть опасно» | Политика `allow` / `confirmation` / `deny` |
| Rollback | Способ вернуться в прежнее состояние | Самый локальный применимый `rollback_mode` |
| Readiness gate | Контрольный список перед стартом | R1–R8 должны дать `READY` |
| Ограничения исполнителя | Что агент реально может обработать | `executor_constraints`, а не имя модели |

## Правило чтения

Сначала прочитай разделы «Коротко для человека» и «Словарь», чтобы понять идею.
Затем используй нормативные разделы и reference-файлы как контракт исполнения.
Если простое объяснение и машинный контракт кажутся противоречивыми, приоритет
у контракта, схемы и явно указанного `done_criteria`.

**Версия 5.1.1 — модульная спецификация: компактное ядро + reference-файлы. Обязательная атомарная декомпозиция для всех планов. Модель-независимый профиль исполнителя. Единая сущность шага (AIO и VERIFY упразднены). Иерархии retry/rollback с классификацией ошибок.**
