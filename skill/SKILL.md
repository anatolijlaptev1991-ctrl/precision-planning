...
description: >-
  ...
  planning, atomic plan или пошаговый план. Превращает обычное описание
  задачи в понятную карту действий с проверками, рисками и откатом, а затем
  создаёт машинно-читаемые YAML/NDJSON-контракты: логические шаги, атомарные
  действия, зависимости, evidence, реестр артефактов и readiness gate.
  Исполнитель описывается только измеримыми ограничениями
  (executor_constraints), а не названием модели.
---

# LAPTEV-PLAN: ПЛАН ЗАДАЧИ, КОТОРЫЙ ПОНЯТЕН И ИСПОЛНИМ
...
Стандартный Plan mode в Claude Code, Codex, OpenCode, Antigravity и похожих
агентских приложениях обычно помогает изучить проект и предложить порядок работы
до начала изменений. Это полезный черновик, но он часто остаётся в диалоге,
UI или свободном тексте конкретного приложения.
...
**Версия 5.1.2 — модульная спецификация: компактное ядро + reference-файлы. Обязательная атомарная декомпозиция для всех планов. Модель-независимый профиль исполнителя. Единая сущность шага (AIO и VERIFY упразднены). Иерархии retry/rollback с классификацией ошибок.**
