...
### Для человека без IT-фона

Представь карту маршрута. Ты пишешь обычными словами, **куда нужно прийти**:
например, «перенести сервис на новую схему токенов и убедиться, что ничего не сломалось».

`laptev-plan` превращает это описание в понятный маршрут:

1. что сделать;
2. в каком порядке;
3. как понять, что каждый шаг действительно выполнен;
4. что может быть опасно;
5. как вернуться назад, если что-то пошло не так.

Скилл **не выполняет задачу сам** и не придумывает требования за пользователя.
Он готовит план, который можно безопасно передать агенту.

### Для автономного агента

Вход: один или несколько `.md`-файлов с требованиями, процедурой или архитектурой.

Выход: директория `Plan_<plan_name>/` с фазами, задачами, атомарными шагами,
зависимостями, рисками, rollback, доказательствами результата, реестром
артефактов, журналами, аудитом и `executor_readiness.yaml`.

**Нормативный приоритет:** YAML/NDJSON-схемы, закрытые списки, поля контрактов
и `done_criteria` важнее пояснительного текста. Если объяснение отвечает на
вопрос «зачем», контракт отвечает на вопрос «что именно сделать и проверить».

## Словарь без лишнего жаргона

| Термин | Что это значит для человека | Что обязан сделать агент |
|---|---|---|
| Логический шаг (`LSH`) | Одна мысль из исходного описания | Сохранить источник через `source_anchor` и `origin` |
| Атомарный шаг | Одно маленькое действие | Выполнить ровно один `action` одним вызовом |
| Evidence | Чек или квитанция результата | Записать exit code, хеши и проверки машинно |
| Артефакт | Подписанный результат для следующего шага | Положить его в `artifacts_registry.yaml` |
| Risk tag | Ярлык возможной опасности | Применить правило allow/confirmation/deny |
| Rollback | Возврат в прежнее состояние | Использовать разрешённый `rollback_mode` |
| Readiness gate | Контрольный список перед стартом | Получить `READY` по проверкам R1–R8 |
| Ограничения исполнителя | Что агент реально может обработать | Использовать `executor_constraints`, а не имя модели |

## Как читать описание

- Разделы **«простое объяснение»** помогают понять идею без специальной подготовки.
- Нормативные разделы описывают точные правила, которые нельзя трактовать «по смыслу».
- `ОБЯЗАН` — пропускать нельзя.
- `ЗАПРЕЩЕНО` — делать нельзя.
- `МОЖЕТ` — разрешённый вариант, но не обязанность.
- Если не хватает данных, правильный результат — `BLOCKED`, а не догадка.
...
## Версия 5.1.1: что изменилось принципиально
...
Текущий npm-пакет содержит не только ядро, но и все reference-файлы v5.1.1:
